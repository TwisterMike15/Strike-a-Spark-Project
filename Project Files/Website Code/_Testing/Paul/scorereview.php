<?php
  ...
  function populate($max_score,$databaseval = "") {
    ...
    echo "<option selected = \"" . $databaseval . "\">" . $databaseval . "</option>"; //default selected option from database
  }
  $allattributes = null;//if using an array
  $visual = null;//
?>
...
<html>
  ...
  <body>
    <center>
      ...
      <form action ="includes\savetosubmit.php" method="POST" onsubmit="return ValidateFields();">

      <div class="title">Poster Design:</div>
      <div class="category">
          <select id = "VisualList">
            <?php populate(4,$visual); ?>
          </select>
